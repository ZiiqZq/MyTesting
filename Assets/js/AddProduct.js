...
function setupAutocomplete() {
    ...
    if (seriesNumberInput) {
        ...
        seriesNumberInput.addEventListener('input', (e) => {
            ...
            const matches = [...new Set(existingProducts
                .filter(p => p.series_number && p.series_number.toLowerCase().includes(value))
...
function createDropdown(id) {
    ...
    dropdown.id = id;
    dropdown.className = 'autocomplete-dropdown hidden absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto';
    return dropdown;
}
...
function renderDropdown(dropdown, matches, onSelect) {
    ...
    matches.slice(0, 10).forEach(product => {
        ...
        item.className = 'px-4 py-3 hover:bg-indigo-50 cursor-pointer border-b border-gray-100 transition-colors duration-150';
        item.innerHTML = `
            <div class="font-semibold text-gray-800">${product.product_name}</div>
            <div class="text-xs text-gray-500">
                ${product.series_number || 'N/A'} - ${product.series || 'No series'}
            </div>
        `;
...
function renderSeriesDropdown(dropdown, matches, onSelect) {
    ...
    unique.slice(0, 10).forEach(series => {
        ...
        item.innerHTML = `
            <div class="font-semibold text-gray-800">${series.series_number}</div>
            <div class="text-xs text-gray-500">${series.series || 'No series name'}</div>
        `;
...
function setupValidationTypeChange() {
    ...
    if (select) {
        select.addEventListener('change', (e) => {
            ...
            const referenceValueInput = document.getElementById('referenceValueInput');
...
async function confirmAddTestType() {
    ...
    if (newTypeName) {
        try {
            const result = await window.electronAPI.addCustomTestType(newTypeName);
            if (result.success) {
                testTypeId = result.id;
                testTypeName = newTypeName;
                await loadTestTypes();
            } else {
                showErrorModal('Failed to create test type: ' + result.error);
                return;
            }
        } catch (err) {
            showErrorModal('Error creating test type: ' + err.message);
            return;
        }
...
function renderTestSequence() {
    ...
    testSequence.forEach((test, index) => {
        ...
        div.className = 'bg-green-100 border-l-4 border-[#354F52] rounded-lg p-4 flex items-center justify-between';
        ...
        div.innerHTML = `
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="font-bold text-gray-800">${test.testTypeName}</h3>
                    <p class="text-sm text-gray-600">
                        ${testParameters[test.testTypeId]?.length || 0} parameters defined
                    </p>
                </div>
            </div>
            <div class="flex gap-2">
                ${index > 0 ? `<button onclick="moveTestUp(${index})" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-lg text-sm cursor-pointer">↑</button>` : ''}
                ${index < testSequence.length - 1 ? `<button onclick="moveTestDown(${index})" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-lg text-sm cursor-pointer">↓</button>` : ''}
                <button onclick="removeTest('${test.id}')" title="Remove Test" class="text-[#800f2f] hover:bg-[#590d22] hover:text-white border-2 border-[#590d22] w-10 h-10 rounded-full cursor-pointer text-xl">-</button>
            </div>
        `;
...
function moveTestUp(index) {
    if (index > 0) {
        [testSequence[index], testSequence[index - 1]] = [testSequence[index - 1], testSequence[index]];
...
function updateParameterTestDropdown() {
    ...
    testSequence.forEach(test => {
        ...
        option.textContent = test.testTypeName;
        select.appendChild(option);
    });
}
...
function openAddParameterModal() {
    ...
    document.getElementById('parameterValidationType').dispatchEvent(new Event('change'));
}
...
function confirmAddParameter() {
    ...
    const validationType = document.getElementById('parameterValidationType').value;
    ...
    if (validationType === 'lsl_usl' && !lsl && !usl) {
        showErrorModal('At least LSL or USL must be specified for LSL/USL validation');
        return;
    }
    ...
    if (testParameters[selectedTest].some(p => p.name === paramName)) {
        showErrorModal('Parameter with this name already exists for this test type');
        return;
    }
...
function renderParameters() {
    ...
    if (!selectedTest || !testParameters[selectedTest] || testParameters[selectedTest].length === 0) {
        list.innerHTML = '<p class="text-center text-gray-400 py-8">No parameters defined for this test</p>';
        return;
    }
    ...
    testParameters[selectedTest].forEach((param, index) => {
        const div = document.createElement('div');
        div.className = 'bg-cyan-50 border border-blue-300 rounded-lg p-4 flex justify-between items-center';
        ...
        div.innerHTML = `
            <div>
                <h4 class="font-bold text-gray-800">${param.name}${param.unit ? ' (' + param.unit + ')' : ''}</h4>
...
async function saveProduct() {
    ...
    if (testSequence.length === 0) {
        showErrorModal('At least one test type must be defined in the test sequence');
        return;
    }
    
    const testTypesWithoutParams = testSequence.filter(test => 
        !testParameters[test.testTypeId] || testParameters[test.testTypeId].length === 0
    );
    
    if (testTypesWithoutParams.length > 0) {
        const testNames = testTypesWithoutParams.map(t => t.testTypeName).join(', ');
        showErrorModal(`The following test types have no parameters: ${testNames}. Add at least one parameter for each.`);
        return;
    }
    ...
    try {
        showLoading(true);
        const result = await window.electronAPI.saveProductWithSequence(productData);
        
        if (result.success) {
            showSuccessModal('Product saved successfully! Product ID: ' + result.productId);
            setTimeout(() => {
...
function showLoading(show) {
    ...
    if (btnSaveProduct) {
        if (show) {
            btnSaveProduct.innerHTML = '<span class="animate-spin">⏳</span> Saving...';
